...
const TodoApp = () => {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div
        data-aos="fade-up"
        data-aos-delay={100}
        className="relative w-full max-w-4xl space-y-8 mx-auto px-4 sm:px-0"
      >
        <section className="rounded-[2rem] border border-white/10 bg-white/5 backdrop-blur-2xl shadow-2xl shadow-slate-950/40 overflow-hidden">
          <div className="p-8 bg-gradient-to-br from-slate-900/90 via-slate-950/80 to-slate-900/95">
            ...
            <p className="mt-4 sm:mt-6 max-w-full sm:max-w-2xl text-slate-400 leading-7">
              Manage your tasks with a sleek glass-style interface, quick
              filters, and effortless reminders for everything you need to get
              done.
            </p>
          </div>
          ...
          <div className="p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-4">
              ...
              <button
                className="w-full rounded-3xl bg-sky-500 px-6 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-400 sm:w-auto"
                onClick={addTask}
              >
                Add task
              </button>
            ...
            <div className="mt-6 rounded-[2rem] border border-white/10 bg-slate-950/80 p-4 shadow-lg shadow-slate-950/30">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                ...
                <button
                  ...
                  className="w-full rounded-full bg-white/5 px-4 py-2 text-sm text-slate-300 transition hover:bg-white/10 sm:w-auto"
                >
                  Clear completed
                </button>
              </div>
              ...
              <div className="mt-5 space-y-3">
                {filteredTasks.length === 0 ? (
                  ...
                  filteredTasks.map((task) => (
                    <div
                      key={task.id}
                      className="flex flex-col gap-3 rounded-3xl border border-white/10 bg-slate-900/70 px-4 py-4 shadow-inner shadow-black/20 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <button
                      ...
                      <p
                        className={`flex-1 min-w-0 text-left text-sm ${task.completed ? "text-slate-500 line-through" : "text-slate-100"}`}
                      >
                        {task.text}
                      </p>
                      <button
                        onClick={() => removeTask(task.id)}
                        className="w-full rounded-2xl px-3 py-2 text-sm text-rose-300 transition hover:bg-rose-300/10 hover:text-rose-200 sm:w-auto"
                      >
                        Delete
                      </button>
                    </div>
